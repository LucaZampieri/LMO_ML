...
class NN(object):
    ...
    def model(self, data, train=False):
        ...
        if CONSIDER_PATCHES==False:
            ...
            # First up-convolution layer
            with tf.name_scope('upconv1'):
                h_upconv1 = upconv2d(h_conv5b, self.W_upconv1, [h_conv5b.get_shape().as_list()[0], 2*h_conv5b.get_shape().as_list()[1], 2*h_conv5b.get_shape().as_list()[2], 512]) + self.b_upconv1
            ...
            # Second up-convolution layer
            with tf.name_scope('upconv2'):
                h_upconv2 = upconv2d(h_conv6b, self.W_upconv2, [h_conv6b.get_shape().as_list()[0], 2*h_conv6b.get_shape().as_list()[1], 2*h_conv6b.get_shape().as_list()[2], 256]) + self.b_upconv2
            ...
            # Third up-convolution layer
            with tf.name_scope('upconv3'):
                h_upconv3 = upconv2d(h_conv7b, self.W_upconv3, [h_conv7b.get_shape().as_list()[0], 2*h_conv7b.get_shape().as_list()[1], 2*h_conv7b.get_shape().as_list()[2], 128]) + self.b_upconv3
            ...
            # Fourth up-convolution layer
            with tf.name_scope('upconv4'):
                h_upconv4 = upconv2d(h_conv8b, self.W_upconv4, [h_conv8b.get_shape().as_list()[0], 2*h_conv8b.get_shape().as_list()[1], 2*h_conv8b.get_shape().as_list()[2], 64]) + self.b_upconv4
    ...
    def get_prediction(self, img):
        ...
        if CONSIDER_PATCHES:
            img_prediction = label_to_img(img.shape[0], img.shape[1], IMG_PATCH_SIZE, IMG_PATCH_SIZE, output_prediction, CONSIDER_PATCHES)
        else:
