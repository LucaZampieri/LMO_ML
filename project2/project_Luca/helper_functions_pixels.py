...
def label_to_img(imgwidth, imgheight, w, h, labels, patches=True):
    ...
    threshold = 0.5
    if patches == True:
        min_ = np.min(labels[:,0])
        max_ = np.max(labels[:,0])
        mean_ = np.mean(labels[:,0])
    ...
    for i in range(0,imgheight,h):
        for j in range(0,imgwidth,w):
            if patches == True:
                ...
                lab = labels[0,j,i,0]
                if lab > threshold: # TODOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
                    l = 0 # 0
                else:
                    l = 1 # 1
                array_labels[j, i] = l
            ...
            idx = idx + 1
    print('max: ',np.max(array_labels))
    print ('        mean  output:',np.mean(np.mean(array_labels)),"#########")
    return array_labels
